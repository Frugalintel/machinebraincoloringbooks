...
CREATE POLICY "Admin full access to campaigns" ON campaigns
  FOR ALL USING (auth.role() = 'authenticated' AND auth.email() LIKE '%admin%'); -- Adjust based on your auth model, usually standard authenticated for admins in simple apps

-- Function to activate a campaign (ensures single active campaign)
