      ...
      <div className="relative z-10 bg-[#111] border border-[#333] rounded-lg p-4 md:p-6 group hover:border-primary/30 transition-colors">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 border-b border-[#222] pb-4 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-mono text-gray-600 uppercase hidden md:inline-block">
          ...
          <button
            ...
            className="text-gray-600 hover:text-red-500 transition-colors self-end md:self-auto p-2"
          >
            <Trash2 size={16} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            {/* Challenge Config */}
            {hasChallenge ? (
              <div className="bg-[#1a1a1a] border border-dashed border-[#333] p-4 rounded space-y-3 animate-in fade-in slide-in-from-top-2">
                {node.challenge?.type === "riddle" && (
                  <>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateChallengeConfig(node.id, "answer", e.target.value)
                      }
                      className="bg-[#111] border-[#333]"
                    />
                  </>
            ...
            <div className="pt-4 border-t border-[#222]">
              ...
              {node.choices.length === 0 ? (
                <div className="text-xs text-gray-500 italic">
                  End of adventure (or configure manually)
                </div>
              ) : (
                <div className="space-y-2">
                  {node.choices.map((choice: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 text-xs bg-[#1a1a1a] p-2 rounded border border-[#333]"
                    >
...
export function StoryNodeEditor({ nodes, onChange }: StoryNodeEditorProps) {
  const [viewMode, setViewMode] = useState<"builder" | "visual" | "json">(
    "builder",
  );
  ...
  const addNode = (
    template: "text" | "choice" | "riddle" | "timer" | "scan" | "end" = "text",
  ) => {
    ...
    const newNode: StoryNode = {
      ...
      choices: [],
      type:
        template === "riddle" || template === "timer" || template === "scan"
          ? "challenge"
          : template === "choice"
            ? "choice"
            : template === "end"
  ...
  const setChallengeType = (
    nodeId: string,
    type: "none" | "riddle" | "timer" | "scan",
  ) => {
    const node = nodes.find((n) => n.id === nodeId);
    if (!node) return;

    if (type === "none") {
      const { challenge: _challenge, ...rest } = node;
            ...
            <div className="sticky bottom-4 z-20 flex justify-center pb-4 pt-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent className="bg-[#1a1a1a] border-[#333] text-white w-56">
                  ...
                  <DropdownMenuItem
                    onClick={() => addNode("end")}
                    className="focus:bg-[#333] cursor-pointer text-primary"
                  >
                    <CheckCircle2 size={14} className="mr-2" /> Adventure Ending
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
