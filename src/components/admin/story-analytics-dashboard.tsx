...
export function StoryAnalyticsDashboard() {
  ...
  const [storyPopularity, setStoryPopularity] = useState<{ name: string; value: number }[]>([]);
    ...
    async function fetchStoryMetrics() {
      ...
      try {
        ...
        popularStories?.forEach((p) => {
            const record = p as { story_id: string; stories?: { title: string }[] | { title: string } };
            const title = Array.isArray(record.stories) ? record.stories[0]?.title : record.stories?.title || 'Unknown';
        ...
        setMetrics({
            ...
            avgCompletionRate: startedCount ? Math.round(((completedCount || 0) / startedCount) * 100) : 0,
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-[#111] border border-[#333] p-6 rounded-lg">
          <h3 className="font-heading text-xl mb-6">Most Popular Stories</h3>
