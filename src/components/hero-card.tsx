  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="relative z-10 mx-auto flex min-h-[calc(100svh-5rem)] w-full max-w-[88rem] flex-col items-center px-5 pb-20 pt-10 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:gap-10 lg:px-16 lg:pb-24 lg:pt-12"
      >
        <div
          data-testid="hero-copy"
          className="flex w-full max-w-xl flex-col items-center text-center lg:max-w-none lg:flex-1 lg:items-start lg:text-left"
        >
          <p className="mb-5 font-mono text-[10px] uppercase tracking-[0.32em] text-(--hero-accent) sm:text-[11px]">
            {brandEyebrow}
          </p>
          <h1
            id="home-hero-heading"
            className="font-heading text-[clamp(3.1rem,14vw,5.75rem)] font-bold uppercase leading-[0.86] tracking-[-0.045em] text-white"
          >
          ...
          <p className="mt-6 max-w-md text-[15px] leading-relaxed text-zinc-400 sm:text-base sm:leading-7">
            {subcopy}
          </p>
          <div className="mt-8 flex flex-col items-center gap-4 lg:items-start">
            <Button
              asChild
              className="h-12 rounded-md bg-(--hero-accent) px-8 font-heading text-sm uppercase tracking-[0.2em] text-white hover:bg-white hover:text-black sm:text-base"
            >
              <Link href={primaryCta.href}>{primaryCta.label}</Link>
            </Button>
            <Link
              href={secondaryCta.href}
              className="inline-flex min-h-11 items-center font-heading text-xs uppercase tracking-[0.24em] text-zinc-400 transition-colors hover:text-white sm:text-sm"
            >
              {secondaryCta.label}
            </Link>
          </div>
          {featureChips.length > 0 ? (
            <ul className="mt-8 flex flex-wrap items-center justify-center lg:justify-start">
              {featureChips.map((chip, index) => (
                <li key={chip.label} className="flex items-center">
                  {index > 0 ? (
                    <span
                      className="mx-4 h-3 w-px bg-zinc-700 sm:mx-5"
                      aria-hidden
                    />
                  ) : null}
        ...
        <div
          ...
          className="hero-cover-stage relative mt-12 flex w-full items-center justify-center lg:mt-0 lg:w-auto lg:flex-1"
        >
          <Link
            href={coverHref}
          ...
          >
            ...
            <span
              ...
              className="absolute -right-2 bottom-8 z-20 rounded-xl bg-white px-4 py-3 text-left text-black shadow-[0_12px_40px_rgba(0,0,0,0.45)] sm:right-0 sm:bottom-10 lg:-right-4 lg:bottom-16"
            >
