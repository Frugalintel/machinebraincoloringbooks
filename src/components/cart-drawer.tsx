...
export function CartDrawer() {
  const { items, addItem, removeItem, updateQuantity, cartTotal, isCartOpen, setIsCartOpen } = useCart();
            ...
            <div className="relative z-10 p-6 border-b border-[#333] flex items-center justify-between bg-[#111] shrink-0">
              <div>
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-primary text-xs font-mono border border-primary px-1.5 py-0.5 rounded-sm bg-primary/10">LOG_01</span>
                </div>
                <div className="flex items-center gap-2 mt-2">
                    <div className="w-1.5 h-1.5 bg-primary animate-pulse"></div>
                    <p className="text-[10px] text-gray-400 font-mono tracking-widest uppercase">System Online • {items.length} Items</p>
                </div>
              </div>
              <button
                onClick={() => setIsCartOpen(false)}
              ...
              {items.length === 0 ? (
                <div className="h-40 flex flex-col items-center justify-center text-center opacity-60">
                    <div className="w-16 h-16 border border-dashed border-[#333] flex items-center justify-center mb-4 rounded-full relative group">
                        <ShoppingBag size={24} className="text-gray-500 group-hover:text-primary transition-colors" />
                    </div>
                    <p className="font-heading text-xl text-white uppercase tracking-widest mb-1">No Items</p>
                    <p className="text-[10px] font-mono text-gray-500 tracking-[0.2em] uppercase">Cart Empty</p>
                </div>
              ) : (
                items.map((item) => (
                  <div key={item.id} className="relative group bg-[#111] p-1 border border-[#222] hover:border-primary/50 transition-colors">
                    <div className="flex gap-4 p-3 bg-[#0f0f0f]">
                        ...
                        <div>
                            <div className="flex justify-between items-start gap-2">
                                <h3 className="font-heading text-base text-white leading-none uppercase tracking-wide group-hover:text-primary transition-colors line-clamp-1">{item.title}</h3>
                                <button 
                            ...
                            <p className="text-[9px] text-gray-500 font-mono tracking-widest mt-1 uppercase line-clamp-1">{item.subtitle || "Standard Edition"}</p>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-2">
                            ...
                            <span className="w-6 text-center text-[10px] font-mono text-white border-x border-[#222] h-4 flex items-center justify-center leading-none">{item.quantity}</span>
                            <button
                                onClick={() => updateQuantity(item.id, item.quantity + 1)}
                                className="w-6 h-6 flex items-center justify-center hover:bg-[#222] text-gray-500 hover:text-white transition-colors"
                            >
                            ...
                            <p className="font-heading text-base text-white tracking-tight">${(item.price * item.quantity).toFixed(2)}</p>
                        </div>
                        </div>
                    </div>
                  </div>
                ...
                <div className="pt-6 border-t border-[#222]">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {suggestedProducts.map(product => (
                            <div key={product.id} className="group relative bg-[#0f0f0f] border border-[#222] hover:border-gray-600 transition-colors p-2">
                                <Link href={`/store/${product.id}`} onClick={() => setIsCartOpen(false)} className="block aspect-[3/4] bg-[#151515] mb-2 relative overflow-hidden">
                                    {product.image_url ? (
                                        <img src={product.image_url} alt={product.title} className="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-opacity" />
                                    ) : (
                                        <div className="w-full h-full flex items-center justify-center bg-[#222] text-gray-700 font-heading text-xl">{product.title.charAt(0)}</div>
                                    )}
                                    {/* Quick Add Overlay */}
                                    <button
                                        onClick={(e) => {
                                ...
                                <div className="space-y-1">
                                    <h5 className="font-heading text-xs text-white uppercase tracking-wide truncate">{product.title}</h5>
                                    <p className="font-mono text-[10px] text-gray-500">${product.price.toFixed(2)}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="space-y-3 mb-6">
                    ...
                    <div className="flex justify-between items-end">
                        <div className="flex flex-col">
                            <span className="font-heading text-xl text-white uppercase tracking-wide">Total</span>
                            <span className="text-[9px] text-primary font-mono tracking-widest">USD // ENCRYPTED</span>
                        </div>
                        <span className="font-heading text-4xl text-primary tracking-tighter leading-none">${cartTotal.toFixed(2)}</span>
                    </div>
                </div>
