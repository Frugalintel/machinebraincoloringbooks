...
export function Hero() {
  ...
  const priceInfo = calculatePrice(
    { price: featuredProduct?.price || 15, discount_percent: featuredProduct?.discount_percent || 0 },
  ...
  const getTextureStyle = () => {
      switch(theme.texture) {
          case 'grid':
              return { backgroundImage: 'linear-gradient(#333 1px, transparent 1px), linear-gradient(90deg, #333 1px, transparent 1px)', backgroundSize: '40px 40px', opacity: 0.1 };
          case 'dots':
              return { backgroundImage: 'radial-gradient(#444 1px, transparent 1px)', backgroundSize: '20px 20px', opacity: 0.2 };
          case 'scanlines':
              return { backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 2px, #000 3px)', opacity: 0.3 };
          case 'noise':
  ...
  return (
    <section className="relative w-full border-b border-[#222] bg-[#111]">
      <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[75vh]">
        
        {/* Left: Value Prop & CTA (7 Columns) */}
        <div className="lg:col-span-7 border-r border-[#222] relative p-8 md:p-16 flex flex-col justify-center overflow-hidden">
            {/* Dynamic Texture Background */}
            <div className="absolute inset-0 pointer-events-none" style={getTextureStyle()}></div>
            ...
            >
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <Link href="/stories">
                        <Button variant="outline" className="h-14 px-8 border-[#222] text-gray-400 hover:text-white hover:border-white font-heading text-xl uppercase tracking-widest rounded-none">
                            Explore Lore
                        </Button>
                    </Link>
                </div>
        ...
        <div className="lg:col-span-5 bg-[#0a0a0a] flex flex-col relative overflow-hidden items-center justify-center p-12">
            <div className="absolute inset-0 bg-[url('/textures/noise.svg')] opacity-5 mix-blend-overlay"></div>
            ...
            >
                <Link href={productLink} className="block w-full h-full">
                    <div className="relative w-full h-full bg-[#111] border border-[#222] shadow-2xl transform group-hover:rotate-y-3 transition-transform duration-500 ease-out preserve-3d">
                        {/* Book Spine Effect */}
                        <div className="absolute -left-4 top-2 bottom-2 w-4 bg-[#0a0a0a] border-l border-y border-[#222] transform -skew-y-6 origin-right"></div>
                            ...
                            {featuredProduct?.image_url ? (
                                ...
                                <>
                                    ...
                                    <div className="flex-1 flex items-center justify-center p-8 relative">
                                        <div className="absolute inset-0 bg-[url('/textures/noise.svg')] opacity-30 mix-blend-multiply"></div>
                            ...
                            <div className="h-20 bg-black/80 p-4 flex justify-between items-center relative z-20 mt-auto">
                                ...
                                <div className="flex flex-col items-end">
                                    {priceInfo.hasDiscount ? (
                                        <>
                                            <span className="text-gray-500 line-through text-sm font-mono">{formatPrice(priceInfo.originalPrice)}</span>
                                            <span className="font-heading text-white text-2xl tracking-tight">{formatPrice(priceInfo.finalPrice)}</span>
                                        </>
                                    ) : (
                                        <span className="font-heading text-white text-2xl tracking-tight">{formatPrice(priceInfo.originalPrice)}</span>
                                    )}
                                </div>
                            </div>
                        </div>
                ...
                {(!isDefault || campaign.isActive) ? <div 
      ...
      <div className="border-t border-[#222] bg-[#0a0a0a] h-10 flex items-center px-4 justify-between text-[10px] font-mono text-gray-600 uppercase tracking-widest">
