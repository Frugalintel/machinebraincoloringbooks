...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
      try {
          ...
          if (error) {
              // Only error if it's not a static product (UUID check approx)
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
            ...
            <div>
                <h2 className="text-3xl font-heading font-bold">EDIT PRODUCT</h2>
                <p className="text-gray-500 font-mono text-sm">Update product details.</p>
            </div>
        </div>
        <button 
            onClick={handleDelete}
            className="flex items-center gap-2 text-red-500 hover:bg-red-900/20 px-4 py-2 rounded transition-colors"
        >
            <Trash2 size={16} />
