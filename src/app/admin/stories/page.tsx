...
import { ADVENTURES_COPY, getStoryTheme } from "@/lib/story-utils";
...
export default function AdminStoriesPage() {
  ...
  const handleDelete = async (story: Story) => {
    ...
    try {
      ...
      await logAdminAction("delete_story", "stories", story.id, {
        title: story.title,
      });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="text-center py-20">
          ...
          <p className="text-gray-500 font-mono">Loading adventures...</p>
        </div>
      ) : (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {filteredStories.map((story) => {
                ...
                return (
                  <div
                    key={story.id}
                    className="group relative bg-[#111] border border-[#333] hover:border-gray-500 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1"
                  >
                    {/* Status Badge */}
                    <div className="absolute top-3 right-3 z-30 flex flex-col items-end gap-2">
                      ...
                      <span
                        className={`text-[10px] uppercase font-bold px-2 py-1 rounded backdrop-blur-md border shadow-sm ${theme.bgAccentLight} ${theme.textAccent} ${theme.borderAccent}/30`}
                      >
                          ...
                          <div className="flex items-center justify-between border-t border-white/10 pt-3">
                            <div className="flex items-center gap-1.5 text-gray-500">
                              ...
                              <span className="text-[10px] font-mono uppercase tracking-wider">
                                {story.estimated_minutes || 15} MIN
                              </span>
                            </div>
                            <div className="flex gap-0.5">
                              {[...Array(5)].map((_, idx) => (
                                <div
                                  key={idx}
                                  className={`w-1 h-2 rounded-sm ${idx < (story.difficulty || 1) ? "bg-primary" : "bg-[#333]"}`}
                                ></div>
                              ))}
                            </div>
                          </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-[#333]">
                    {filteredStories.map((story) => (
                      ...
                      >
                        <td className="p-4">
                          <div className="w-16 h-12 bg-[#222] rounded overflow-hidden relative">
                            {story.cover_url ? (
                              <img
                                src={story.cover_url}
                                alt={story.title}
                        ...
                        <td className="p-4">
                          <div className="font-heading font-bold text-white text-sm truncate max-w-[200px]">
                            {story.title}
                          </div>
                          <div className="text-[10px] text-gray-500 font-mono mt-1">
                            Nodes: {story.content?.length || 0}
                          </div>
                        </td>
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="flex gap-0.5">
                              {[...Array(5)].map((_, idx) => (
                                <div
                                  key={idx}
                                  className={`w-1 h-1.5 rounded-[1px] ${idx < (story.difficulty || 1) ? "bg-primary" : "bg-[#333]"}`}
                                ></div>
                              ))}
                            </div>
                          </div>
