...
function AuthPageContent() {
      ...
      if (isLogin) {
        ...
        if (error) {
          if (
            ...
            setIsLogin(true);
            setErrorMsg(
              "Account already exists. Please enter your password to login.",
            );
            setLoading(false);
            return;
          }
  ...
  const SocialButton = ({
    ...
    return (
      ...
      >
        {isLastUsed ? (
          <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 text-[8px] bg-primary text-black px-1.5 font-mono uppercase tracking-wide">
            Last Used
          </span>
        ) : null}
        <Icon
          size={18}
          className={`mr-2 ${isLastUsed ? "text-primary group-hover:text-black" : ""}`}
        />
    ...
    <form onSubmit={handleAuth} className="space-y-6 mb-6">
      <div className="space-y-2">
        ...
        {lastProvider === "email" && isLogin ? (
          <div className="text-[9px] text-primary uppercase tracking-widest font-mono text-right">
            Last Used Method
          </div>
        ) : null}
      </div>
