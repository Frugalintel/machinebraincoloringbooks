    ...
    try {
      ...
      if (codeData.unlocks_type === "collectible" && codeData.unlocks_id) {
        ...
        if (targetSetId) {
          ...
          logger.warn("Unlocked item not found in local sets:", {
            unlocks_id: codeData.unlocks_id,
          });
      ...
      <div className="bg-[#111] border-b border-[#333] py-12 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          <p className="font-mono text-sm text-gray-500 tracking-widest uppercase max-w-xl">
            Your digital rewards from coloring books.
          </p>
        </div>
      </div>
          ...
          {Array.isArray(collectionSets) &&
            collectionSets.map((set) => {
              ...
              return (
                <div
                  key={set.id}
                  className="border border-[#333] bg-[#0a0a0a] p-6 md:p-8 relative overflow-hidden group"
                >
                  {/* Progress Bar */}
                  ...
                  <div className="flex justify-between items-start mb-8">
                    <div>
                      <h3 className="font-heading text-3xl text-white uppercase tracking-wide mb-2">
                        {set.title}
                      </h3>
                      <div className="flex items-center gap-2 text-xs font-mono text-gray-500 uppercase tracking-widest">
                        ...
                        <span className="text-primary">
                          [{set.collected.length}/{set.total}]
                        </span>
                      </div>
                    </div>
                    {isComplete ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {set.items.map((item) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`aspect-square border ${isCollected ? "border-primary/50 bg-[#151515]" : "border-[#333] border-dashed bg-transparent"} relative flex flex-col items-center justify-center p-4 group/item transition-all hover:border-primary cursor-pointer h-full`}
                          >
                            {isCollected ? (
                              <>
                                <div className="absolute inset-0 z-0">
                                  <TrophyCanvas
                                    ...
                                    isInteractive={false}
                                    unlockedAt={
                                      collectibleTimestamps[item.id]?.unlockedAt
                                    }
                                    lastPolishedAt={
                                      collectibleTimestamps[item.id]
                                        ?.lastPolishedAt
                                    }
                                  />
                                </div>
                                <span className="absolute bottom-2 left-0 right-0 text-[10px] font-mono text-gray-400 uppercase text-center z-10 group-hover/item:text-white transition-colors">
                                  {item.name}
                                </span>
                              ...
                              <>
                                ...
                                <span className="text-[10px] font-mono text-gray-700 uppercase text-center group-hover/item:text-gray-500 transition-colors">
                                  Locked
                                </span>
                              </>
                            )}
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-[#0a0a0a] border border-[#333] p-6 md:p-8 relative">
            ...
            <h3 className="font-heading text-xl text-white mb-6 uppercase flex items-center gap-2">
              <Scan size={18} className="text-primary" /> Enter a code
            </h3>

            <form onSubmit={handleClaim} className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest text-primary font-mono">
                  Code
                </label>
                <Input
                  value={code}
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full h-12 bg-[#222] hover:bg-primary hover:text-black text-white font-heading uppercase tracking-widest rounded-none transition-all"
              >
                {loading ? (
          ...
          <AnimatePresence>
            {unlockedReward ? (
              ...
              >
                ...
                <p className="text-primary font-mono text-sm uppercase tracking-widest mb-6">
                  {unlockedReward}
                </p>
                ...
                <div className="w-full aspect-square bg-[#0a0a0a] border border-[#333] relative mb-4 overflow-hidden">
                  ...
                  <TrophyCanvas
                    ...
                    isInteractive={true}
                  />
                  <div className="absolute bottom-4 left-0 right-0 text-center text-[10px] font-mono text-gray-500 uppercase pointer-events-none">
                    Drag to Rotate
                  </div>
                </div>
